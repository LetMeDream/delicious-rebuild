...
const AceternityGrid = (
  ...
  return (
		<div className=" mx-auto max-w-[80%] w-[80%] px-8">
			...
			{recipes.map((recipe, idx) => { 
				return (
					...
					>
						...
						<div className=" cursor-pointer rounded-2xl h-full w-full p-4 overflow-hidden bg-gradient-to-br from-[#d0b5a6] to-[#d0b5a6]/[0.2] border border-[hsl(21,31%,30%)] relative z-50">
							<div className="relative z-50">
								<Card className='flex flex-col bg-transparent border-none shadow-none'>
									<CardTitle className=' h-[40px] text-white group-hover:text-[hsl(21,31%,0%)] transition duration-500 cursor-pointer'>
										<span className="text-sm text-center font-semibold">{recipe.title}</span>
									</CardTitle>
									<div className=' overflow-hidden'>
										<CardContent 
											style={{'--image-url': `url(${recipe?.image})`} as CustomStyle} 
											className={`flex h-[200px] p-5 py-5 bg-[image:var(--image-url)] bg-cover hover:scale-[102%] transition-all duration-500 mt-2 cursor-pointer`}>
